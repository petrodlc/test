...
Two-dimensional table, storing raman shifts each spectrum, equivalent to [`spectra.data[:, 0, :]`][data].
...
#### Returns
{: .no_toc }
- shift ([`numpy.ndarray`][ndarray]{:target="_blank"}(`numpy.float`) of shape (*m*, *n*)):\
  a copy of the actual raman shift stored in the [`data`][data] property
